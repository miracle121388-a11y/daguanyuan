"""r8 construction system on the reviewed garden footprints.
...
def lattice(b,x,y,z,w=2,h=2.6,pattern='grid',paper=True):
    ...
    rows=max(3,round(h/.52));cols=max(2,round(w/.46))
...
def hall(b,x=0,y=6,w=18,d=8,h=4.4,openhall=False,detail=True,bays=3,style='plain',rolled=False,thatch=False):
    ...
    for i in range(bays+1):
        ...
        for sy in [-1,1]:
            ...
            b.rod('wood',(cx,cy,.69),(cx,cy,coltop),.16 if w>20 else .12,16,r2=.14 if w>20 else .10)
    ...
    for sy in [-1,1]:
        b.box('darkwood',(x,y+sy*(d/2+.36),coltop-.20),(w+.35,.30,.12))
        ...
        for sy in [-1,1]:
            for i in range(bays):
                ...
                if i==bays//2:
                    ...
                    # Three independent door leaves, with recessed panels and a common transom.
                    leaf_width=(spacing-.38)/3
                    for k in range(3):
                        xx=cx+(k-1)*leaf_width
                        def leaf():
                            b.box('wood',(0,0,.89),(leaf_width-.04,.13,.62))
                            b.box('furniture',(0,-sy*.05,.88),(leaf_width-.19,.08,.40))
                            lattice(b,0,0,1.25+(screen_h-.18)/2,leaf_width-.055,screen_h-.18,'grid',paper=k==1 or sy==1)
                        transform(b,leaf,(xx,y+sy*inner,0),(k-1)*.23 if sy==-1 else 0)
                    b.box('darkwood',(cx,y+sy*inner,h-.16),(spacing-.22,.17,.17))
        for sx in [-1,1]:
    ...
    # Continuous purlin and paired fascia form a visible transition beneath the tiles.
    for sy in [-1,1]:
        for j in range(max(3,round(w/.32))):
            xx=x-w/2+(j+.5)*w/max(3,round(w/.32))
            b.rod('wood',(xx,y+sy*(d/2+.20),h+.77),(xx,y+sy*(d/2+1.07),h+.88),.038,8)
...
def roof(b,x,y,z,w,d,h=2.25,detail=True,rolled=False,thatch=False):
    if thatch:return r7.roof(b,x,y,z,w,d,h,detail,rolled,True)
    rise=max(h,d*(.30 if rolled else .40));ridge=max(.2,(w-d)*.5)
    hw=w/2+1.05;hd=d/2+1.05;start=previous._skirt
    def point(u,t,side,lift=0,hip=False):
        profile=math.cos(t*math.pi/2)**1.35 if rolled else (1-t)**1.45
        zz=z+rise*profile+.20*t**9+.54*t**7*abs(u)**10+lift
        return (x+side*(ridge+(hw-ridge)*t),y+u*t*hd,zz) if hip else (x+u*(ridge+(hw-ridge)*t),y+side*t*hd,zz)
    for hip in [False,True]:
        rows=max(8,round((hw-ridge if hip else hd)/.33))
        for side in [-1,1]:
            ts=[start+(1-start)*j/rows for j in range(rows+1)]
            for j in range(rows):
                t,t2=ts[j:j+2];span=t2*hd if hip else ridge+(hw-ridge)*t2
                flip=(side>0) if hip else (side<0)
                b.mesh('roof',[point(u,tt,side,0,hip) for tt in [t,t2] for u in [-1,1]],[(2,3,1,0) if flip else (0,1,3,2)])
                if not detail:continue
                n=max(2,round(2*span/.27))
                for k in range(n):
                    u=-1+2*k/n;du=2/n
                    # A shallow semicircular cover tile overlaps the course below.
                    mat='tilelight' if (j//3+k//5)%13==0 else 'tile'
                    vs=[point(u+du*q/3,tt,side,.022+.075*math.sin(q*math.pi/3)+(0.012 if row==1 else 0),hip) for row,tt in enumerate([max(start,t-.006),t2]) for q in range(4)]
                    fs=[(q,q+1,q+5,q+4) for q in range(3)]
                    vs.extend([tuple((a,b,c-.032)) for a,b,c in vs[4:8]])
                    fs.extend((q+4,q+5,q+9,q+8) for q in range(3))
                    b.mesh(mat,vs,[tuple(reversed(face)) for face in fs] if flip else fs)
            if not hip:
                for i in range(max(4,round(w/.30))+1):
                    p=Vector(point(-1+2*i/max(4,round(w/.30)),1,side,.02))
                    b.rod('tiledark',p-Vector((0,side*.035,0)),p+Vector((0,side*.09,0)),.057,8)
                b.box('darkwood',(x,y+side*(hd-.07),z-.015),(w+1.95,.15,.19))
                b.box('wood',(x,y+side*(hd-.11),z+.13),(w+1.96,.18,.09))
            else:b.box('darkwood',(x+side*(hw-.07),y,z),(.15,d+1.95,.19))
    for sx in [-1,1]:
        for sy in [-1,1]:
            pts=[point(sx,start+(1-start)*j/16,sy,.16) for j in range(17)]
            for a,c in zip(pts,pts[1:]):b.rod('tiledark',a,c,.105,10)
            end=Vector(pts[-1]);b.rod('tiledark',end,end+Vector((sx*.25,sy*.21,.31)),.09,8,r2=.033)
    if not start and not rolled:
        b.rod('tiledark',(x-ridge-.2,y,z+rise+.15),(x+ridge+.2,y,z+rise+.15),.14,12)
        for sx in [-1,1]:
            b.rod('tiledark',(x+sx*ridge,y,z+rise+.18),(x+sx*(ridge+.26),y,z+rise+.66),.10,10,r2=.048)
...
def tower(b):
    ...
    # Larger closely fitted flagstones read as a continuous dressed terrace.
    # Their upward faces and UV survive every LOD instead of becoming shards.
    for row in range(7):
        start=-17.55;offset=.90 if row%2 else 0
        while start<17.55-.02:
            width=min(1.95-offset,17.55-start);offset=0
            b.box('paving',(start+width/2,-16.43+(row+.5)*1.21,.572),(width-.016,1.194,.060))
            start+=width
    for sx in [-1,1]:
...
def install():
    ...
    world.tower=tower
    from r8_courtyard import courtyard,bamboo
    world.courtyard=courtyard;arch.bamboo=bamboo
