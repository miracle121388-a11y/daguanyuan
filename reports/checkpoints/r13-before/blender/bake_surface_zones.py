...
if planting_path.exists():
 ...
 for key,beds in planting['courts'].items():
  ...
  for cx,cy,rx,ry,angle in beds:
   ...
   stamp=np.clip((edge+0.23-radius)/.37,0,1)*(.79+.16*np.sin(x*2.8+y*1.7)**2)
...
shoulder=np.exp(-(np.maximum(0,road-1.2)/2.5)**2)*(.80+.2*np.sin(x*.77+y*.57)**2)
