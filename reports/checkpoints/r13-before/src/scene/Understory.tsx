...
export default function Understory({manifest}:{manifest:Manifest}){
 ...
 const meshes=useMemo(()=>sources.flatMap((g,species)=>{
  ...
  g.scene.traverse(ob=>{
   ...
   material.emissiveMap=material.map;material.emissive.set('#526b35');material.emissiveIntensity=.10;
   const mesh=new THREE.InstancedMesh(geometry,material,64);mesh.userData.species=species;mesh.name='Source fern '+species;mesh.count=0;mesh.frustumCulled=false;mesh.receiveShadow=quality==='high';mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);result.push(mesh);
  });return result;
 }),[sources,quality]);
 ...
 useFrame(({camera})=>{
  ...
  last.current.copy(camera.position);lastDirection.current.copy(camera.quaternion);const far=quality==='high'?46:29,budget=quality==='high'?64:28;
