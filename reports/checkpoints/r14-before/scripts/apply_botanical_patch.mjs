...
for(const [file,patch] of jobs){
 const doc=await io.read(file),source=await io.read('assets/processed/botanical-patch-r12-fix2/'+patch),scene=doc.getRoot().listScenes()[0],before=semantics(doc);
...
writeFileSync('reports/acceptance/r12-fix2-botanical-export-preservation.json',JSON.stringify({at:new Date().toISOString(),revision:layout.assetRevision,rows},null,2));
