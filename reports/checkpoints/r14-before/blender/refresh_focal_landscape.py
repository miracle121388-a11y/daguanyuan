...
for p in layout['places']:
 ...
 for ob in list(root.children):
  if ob.type=='MESH' and ob not in protected and (ob.get('focalReviewPatch') or (ob.data.materials and all(botanical(m.name) for m in ob.data.materials))):removed.append(ob.name);bpy.data.objects.remove(ob,do_unlink=True)
...
for ob in list(root.children):
 if ob.get('focalReviewPatch') and ob not in protected:bpy.data.objects.remove(ob,do_unlink=True)
