...
def attach(mask):
 ...
 edge=node('ShaderNodeMapRange','BedEdge');edge.interpolation_type='SMOOTHSTEP';edge.inputs['From Min'].default_value=.12;edge.inputs['From Max'].default_value=.90;links.new(separate.outputs[0],edge.inputs['Value'])
 ...
 factor=node('ShaderNodeMath','Factor');factor.operation='MINIMUM';factor.inputs[1].default_value=.91;links.new(extent.outputs[0],factor.inputs[0])
