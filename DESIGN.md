...
品牌图标采用本项目原创 SVG：绛红圆角底，暖白月洞门、翘檐亭榭与一线曲水。图内不排文字，名称在图外显示，避免字体、竖排与边框造成裁切。源文件为 `src/assets/garden-mark.svg`，页眉、加载页和浏览器标签页共用；不引用外部图标、字体或历史印章。来源与 SHA-256 记录在 `reports/acceptance/brand-mark-20260921/design.json`。

页眉图标桌面 40px、平板 36px、手机 32px，加载页 64px，保持正方形且不收缩。手机将“大观园／入梦”分两行；601–760px 的窄平板只显示图标，为主导航保留位置。首页按钮保留完整无障碍名称与键盘焦点，手机按钮高度不少于 44px。样式集中在 `src/styles/brand.css`，旧品牌竖排和双线框规则已移除。

以下为历史园景和界面记录；其中温纸底、青绿按钮、文字印章及旧“当前版本”描述不再作为现行 UI 规范。
