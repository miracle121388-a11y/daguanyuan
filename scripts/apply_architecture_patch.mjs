...
const revision=JSON.parse(readFileSync('config/qing.palette.json','utf8')).revision;
...
const replaced=n=>!!placeId(n)&&!!n.getMesh()&&(n.getExtras().qingArchitecture||n.getMesh().listPrimitives().every(p=>/^(Baked_Mobile_(?!landscape)|roof$|screen$|lantern$|silk$)/.test(p.getMaterial()?.getName()??'')));
