...
sunwen=(R/'config/garden.sunwen.json').exists()
report_revision='r17' if sunwen else config['revision']
rows=[]
...
for place in ['daguanyuan_gate','xiaoxiangguan','daguanlou','hengwuyuan']:
 ...
 for material in doc['materials']:
  ...
  if material.get('extras',{}).get('sunwenSurface'):source=R/material['extras']['sunwenSurface']['file']
...
(R/'reports/acceptance'/f'{report_revision}-craft-pixel-verification.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
...
(R/'reports/acceptance'/f'{report_revision}-fern-alpha.json').write_text(json.dumps(alpha_report,indent=2),encoding='utf-8')
...
leaf_rows=[]
alpha_models=[('fern-0','fern_02'),('fern-1','fern_02')]
if not sunwen:alpha_models += [('broadleaf','island_tree_01_leaves'),('broadleaf-low','island_tree_01_leaves'),('broadleaf-2','island_tree_02_leaves'),('pine','pine_sapling_small_twig'),('shrub','island_tree_01_leaves')]
for model,stem in alpha_models:
...
leaf_report={'checks':len(leaf_rows),'passed':len(leaf_rows)==len(alpha_models) and all(r['passed'] for r in leaf_rows),'materials':leaf_rows,'method':'Compare opaque leaf-area RGB in every delivered alpha-bearing fern/tree/shrub LOD against the current packed source PNG, with2/255 mean-channel tolerance; require actual transparent and opaque alpha pixels.'}
(R/'reports/acceptance'/f'{report_revision}-vegetation-pixel-verification.json').write_text(json.dumps(leaf_report,indent=2),encoding='utf-8')
