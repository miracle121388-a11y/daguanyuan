...
try{
 await page.goto(base);await b('剧情画卷').click();await page.locator('.story-dream-tools .dream-entry').click();await page.getByText('空白的一页，等你入梦',{exact:true}).waitFor();await shot('mobile-empty-album');
 ...
 await b('剧情画卷').click();await page.locator('.story-dream-tools .dream-entry').click();await b('刷新画册').click();await b('继续查询原画').click();
 ...
 await b('关闭梦藏').click();await b('关闭剧情画卷').click();await b('游园设置').click();await page.getByLabel('避免剧透',{exact:true}).check();await page.getByRole('slider',{name:'阅读进度'}).fill('40');await b('关闭设置').click();
 ...
 await b('剧情画卷').click();await page.locator('.story-dream-tools .dream-entry').click();check(await page.locator('.dream-painting').count()===0,'spoiler setting hides later collected and unfinished artwork');
