...
export interface ScenePlace {id:string;name:string;position:Vec3;cameraPosition:Vec3;cameraTarget:Vec3;model:string;mobileModel?:string;featured:boolean;hotspots:{id:string;name:string;position:Vec3}[];boundingBox:{min:Vec3;max:Vec3}}
export interface Manifest {overview:string;overviewCamera:{position:Vec3;target:Vec3};places:ScenePlace[];pathNodes:{id:string;position:Vec3}[];pathEdges:{from:string;to:string;kind:string}[];lake:{center:number[];radius:number[]}}
