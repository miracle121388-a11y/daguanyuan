...
function Overview({manifest,detailId,onReady}:{manifest:Manifest;detailId:string|null;onReady:(ready:boolean)=>void}){
 ...
 useEffect(()=>{
  ...
  clearTimeout(pending.get(scene));pending.delete(scene);
  // Strict Mode replays effects. Release only after a real unmount, otherwise
  // its replay would reuse a batch whose internal GPU textures are already null.
  return()=>{pending.set(scene,setTimeout(()=>{scene.traverse(o=>{if(o instanceof THREE.BatchedMesh)o.dispose()});pending.delete(scene)},0))};
 },[scene]);
