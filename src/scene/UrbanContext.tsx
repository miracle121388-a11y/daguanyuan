...
import {instances,model} from '../../public/urban-context.json';

/** The same linked courtyard modules and transforms as the editable master. */
export default function UrbanContext(){
 const gltf=useGLTF(import.meta.env.BASE_URL+model);
 const scene=useMemo(()=>{
 const group=new THREE.Group();group.name='宁荣府邸与京城街坊';
  const transforms=new Map<string,typeof instances>();
  for(const row of instances){const rows=transforms.get(row.kind)??[];rows.push(row);transforms.set(row.kind,rows)}
