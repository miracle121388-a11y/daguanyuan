...
import {BookOpen, Expand, LocateFixed, MessageCircle, Moon, Sun, Users, Clapperboard, Check, SlidersHorizontal} from 'lucide-react';
...
export default function SimulationStage() {
  ...
  return <div className={'sim-stage ' + (s.immersive ? 'immersive' : '')} aria-label="场景推演互动">
    <div className="sim-stage-top"><div className="sim-stage-time">{night ? <Moon size={15}/> : <Sun size={15}/>}<span>{clockLabel(world.minutes)}</span><small>{s.journal?.active === 'if' ? 'IF 世界' : '主世界'}</small></div><button className="sim-stage-book" title={s.immersive&&dreamNotice?'有画卷动态，打开手记查看':undefined} onClick={() => useSimulation.setState({immersive: !s.immersive})}>{s.immersive ? <BookOpen size={16}/> : <Expand size={16}/>}<span>{s.immersive ? '打开推演手记' : '入园沉浸'}</span>{s.immersive&&dreamNotice&&<i className="sim-keepsake-dot" aria-hidden="true"/>}</button></div>
    ...
    <div className="sim-stage-bottom">
      {s.phase === 'ready' && moment && <button className="sim-moment" onClick={() => useSimulation.setState({ifComposerOpen:false,recordView:'events',immersive:false,automatic:false})}><Check size={16}/><span><small>这一刻，已留在故事里</small><strong>{moment.text}</strong></span><BookOpen size={16}/></button>}
      ...
      <Disclosure className="sim-stage-views" label={<><SlidersHorizontal size={15}/><span>镜头与速度</span></>}><button disabled={!actor} aria-pressed={!!actor && s.cameraMode === 'close'} onClick={() => useSimulation.setState({cameraMode:'close',focusRevision:s.focusRevision+1})}><Users size={15}/>临场</button><button disabled={!actor} aria-pressed={!!actor && s.cameraMode === 'follow'} onClick={() => useSimulation.setState({cameraMode:'follow',focusRevision:s.focusRevision+1})}><LocateFixed size={15}/>跟随</button><button aria-pressed={!actor} onClick={() => { s.focus(null); useGarden.getState().home(); }}>全园</button><button aria-pressed={s.director} onClick={() => useSimulation.setState({director:!s.director,...(!s.director&&s.actor?{focused:s.actor,focusRevision:s.focusRevision+1}:{})})}><Clapperboard size={14}/>剧情跟拍</button><SimulationSpeed/></Disclosure>
      {s.immersive && <div className="sim-stage-dock"><SimulationRunControls compact/>{s.phase !== 'ready' && <button className="sim-stage-cancel" onClick={s.cancel}>撤销本步</button>}<p role="status">{s.error || (!s.sceneReady ? '园林载入中…' : s.paused ? '已暂停' : s.phase !== 'ready' ? '人物正在行动…' : '这一刻已自动保存')}</p></div>}
    </div>
  </div>;
}
