...
export default function SimulationStage() {
  ...
  return <div className={'sim-stage ' + (s.immersive ? 'immersive' : '')} aria-label="场景推演互动">
    <div className="sim-stage-top"><div className="sim-stage-time">{night ? <Moon size={15}/> : <Sun size={15}/>}<span>{clockLabel(world.minutes)}</span><small>{editionFor(s.editionId, data.editionCatalog).shortTitle} · {s.journal?.active === 'if' ? 'IF 世界' : '主世界'}</small></div><button className="sim-stage-book" onClick={() => useSimulation.setState({immersive: !s.immersive})}>{s.immersive ? <BookOpen size={16}/> : <Expand size={16}/>}<span>{s.immersive ? '打开推演手记' : '入园沉浸'}</span></button></div>
    <div className="sim-stage-roster" role="group" aria-label="选择场景人物">{agentIds.map(id => <button key={id} disabled={!world.agents[id].alive} title={world.agents[id].alive ? undefined : "此幕已退场"} aria-pressed={s.focused === id} onClick={() => s.focus(id)}><i style={{background: agentColors[id]}}/>{world.agents[id].name}</button>)}</div>
