...
export function MotionComic({cue, node, painting}: {cue: ComicCue; node: StoryNode; painting?: DreamEntry}) {
  ...
  return <dialog ref={ref} className={'motion-comic ' + (animating ? 'is-playing' : 'is-paused') + (motion ? '' : ' reduced-motion')} aria-labelledby="comic-title" onCancel={e => {e.preventDefault(); close();}} onKeyDown={e => {if (e.target instanceof HTMLInputElement || e.target instanceof HTMLSelectElement) return; if (e.key === 'ArrowRight') {e.preventDefault(); advance(frame + 1);} if (e.key === 'ArrowLeft') {e.preventDefault(); advance(frame - 1);}}}>
    ...
    <header className="comic-heading"><div><h2 id="comic-title">{node.title}</h2><p>{edition.shortTitle} · 第{node.chapter}回 · {painting ? `剧情新绘 · ${painting.job.moment.branch === 'if' ? 'IF线' : '主世界'}` : isIf ? 'IF 转折 · 借景演绎' : '全彩动态漫画'}</p></div><button aria-label="关闭动态漫画" onClick={close}><X size={22}/></button></header>
