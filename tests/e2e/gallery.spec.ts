...
test('gallery retries a failed manifest and distinguishes spoiler-empty results',async({page})=>{
 ...
 await page.getByRole('button',{name:'游园设置',exact:true}).click();await page.getByRole('checkbox',{name:'避免剧透',exact:true}).check();await page.getByRole('slider',{name:'阅读进度'}).fill('1');await page.getByRole('button',{name:'关闭设置'}).click();await page.getByRole('button',{name:'园景图录',exact:true}).click();await expect(page.getByText('当前阅读进度内暂无园景图片。可在游园设置中调整阅读进度。')).toBeVisible();
});
