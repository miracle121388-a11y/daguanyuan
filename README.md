...
视觉层现以孙温《红楼梦》绘本为标准：灰青瓦、朱红与石绿彩绘、粉墙、淡金，以及青绿为底、百花映衬的层叠花木。青石院坪、卵石小径、苔地和湿润水岸取代大面积裸土。八类树形的近景三维网格与远景烘焙来自同一可编辑模型。图录展示 15 幅绘本原图，可跳转到对应三维景点；详见 [孙温美术依据](docs/SUNWEN_STYLE.md)。
...
从导航进入“世界推演”，在现有三维园林中运行宝玉、黛玉、宝钗与王熙凤。支持自然语言 IF、持续计划与消息打断、沿既有道路进入四处庭院、对话与个人记忆、玩家托付、人物追问、消息传播图、主世界/IF/历史线切换、同 Tick 对照、自动推演、暂停、快照恢复和本地存档。可切换临场与沉浸模式，导出带实际事件依据的纪要。默认规则模式无需 API Key；可选 DeepSeek-V4.1-Flash 已适配服务端决策与小结，按“推演方式与说明”切换并填写推演访问口令。推演内容均为虚构，与原著资料分开。

### 亲自入局

1. 进入“世界推演”，点击场景人物旁的“交谈入局”，或手记中的“入局互动”。
2. **与他交谈**：自由输入，可叙话、宽慰或直言。人物只看自己的记忆与最近四轮对话；回应即时存档，不推进时辰。默认本地规则，DeepSeek用于更自由的交谈。
3. **临场抉择**：根据当前情境选择怎样回应。可以先“从此刻另开一线”，再尝试不同安排；选择留下快照，可回到选择之前。
4. **邀人小聚**：选已有庭院，邀请两到四人联句或品茗。点击“继续这场小聚”，看人物沿园路赴约；到齐并完成活动后才留下共同记忆、改变相互信任。

新流程验收：`npm run test:simulation:participation`。真实交谈验收另设 `PARTICIPATION_MODEL=1`，会使用当前服务配置发出两次模型请求。

使用与扩展见 [推演系统说明](docs/simulation-engine.md)，两项目源码研究与机制落地见 [OpenStory / MiroFish 对照](docs/simulation-reference-study.md)。启动页面后运行 `npm run test:simulation` 验证完整浏览器链路。
...
| `public/art/` | 15 幅孙温绘本原图与缩略图，本地 WebP 及来源、哈希记录 |
| `output/imagegen/daguanyuan-reference-20260910/` | 历史 AI 参考档案，已退出当前建模依据与网页图录 |
| `references/sunwen/` | 固定上游版本的 42 幅孙温原图、203 图目录及来源校验 |
| `config/garden.sunwen.json` | 孙温绘本建筑配色、树形和疏密规则；详见 [美术依据](docs/SUNWEN_STYLE.md) |
