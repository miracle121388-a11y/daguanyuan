...
2. 默认“本地规则”不需要 API Key。直接运行下一 Tick，或输入假设并点击“创建 IF 世界”。
...
| `src/scene/SimulationActors.tsx` | 三维小像、沿路动画、转身、对白、跟随镜头 |
...
本仓库原先没有可复用的人形网格。新人物采用共享的代码几何小像与摆臂组件，用于三维舞台定位；不是角色考据造像或演员肖像，不需要外部下载。现有建筑、植被、GLB、Blender 母场景和 `Manual_Adjustments` 均保留。
...
- “临场”切到人物近景，“跟随”沿其行进追踪，“全园”回到全景。移动与活动由同一套小像表达，读写有持书动作，休息有低坐姿态。
...
启动本地服务后运行 `npm run test:simulation`：验证暂停、到达、对白、记忆、主/IF差异、恢复、刷新、撤销和390/320手机视口。开发服务使用 `npm run dev -- --mode test` 时还会测量实际移动网格和恢复位置；正式构建不含这些测试入口。可用 `APP_URL` 指向正式包或线上服务，`SIM_REPORT_DIR` 指定证据目录，默认写入 `output/playwright/simulation/`。浏览器默认使用 Edge / ANGLE SwiftShader 软件渲染；设置 `SIM_SOFTWARE=0` 使用 Edge 默认渲染器。两者均为浏览器模拟，不能用于证明真实手机帧率。新增浏览器流程还检查院内托付、读书、追问、消息图、报告下载、同刻对照、历史线保存交换及手机沉浸。院内结构通行检查使用 `npm run simulation:space`；原有游园交互另由 `npm run test:e2e` 回归。
