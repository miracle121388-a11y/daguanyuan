...
2026-09-18 与孙温园林 r21 合并后，五幅旧场景源图原字节归档至 `references/dream-scene-sources/`，由 `config/dream.references.json` 的 `sceneSourceCatalog` 指向归档目录。来源记录同时保存当前归档路径、原公开路径、原目录及远端提交；`origin` 仍是生成时的原始说明。21份梦绢参考图的字节、编号和哈希保持不变；重新整理参考时从归档读取，新的路径会写入新产物的来源说明。园景图录及三维建模继续使用孙温绘本，归档不会被发布为园景图录或历史绘画。
