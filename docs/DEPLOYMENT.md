...
[在线游园](https://daguanyuan-rumeng.zeabur.app/)。验证时间 2026-09-13T05:16:22.339160+00:00，运行版本 `spatial-garden-20260913-r15`，部署ID `6aa6308d738cb98df77440da`。
...
CLI状态为RUNNING，域名PROVISIONED，HTTPS健康检查、CSP、18个公开文件（包含当前JS/CSS）的SHA-256及线上桌面/手机模拟正式构建检查通过。本机默认网络的正式网页检查通过。本轮成功的浏览器检查是否使用临时解析覆盖：False。TLS验证保持开启，没有修改系统网络设置。本机条件不能代表每个访客的网络。
...
发布包是 `.deploy/` 内按时间创建的独立目录，不能上传整个 `.deploy/`。本轮目录 `.deploy/reference-20260913-050752-139260`，合计 44.54 MiB。只复制当前Vite清单与验证过的公开资源，避免Windows旧构建快照残留；不包含Blender、原文缓存、源资源下载和登录凭据。
